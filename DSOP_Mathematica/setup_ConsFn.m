...
{\[GothicA]GridMin,\[GothicA]GridMax,\[GothicA]GridN} = {0.001,20.,10};  (* Saving gridpoints parameters *)

NoMoM = True;    (*Default: do not use Method of Moderation*)
Sdfix = True;    (*Default: st. dev. of income shocks fixed during the working age.*)

\[Mho] = 0.5/100;                         (* Probability of zero income during working age*)
\[Mho]Retire = 0+Boole[NoMoM==False]*\[Mho]/10; (* Probability of zero income after retirement *)

NumOf\[Theta]ShockPoints = 7;  (* Number of points of the discrete approximation to lognormal distribution of transitory shocks  *)
NumOf\[CapitalPsi]ShockPoints = 7; (* Number of points of the discrete approximation to lognormal distribution of permanent shocks *)


(* NOTE: 
   If MoM is used, \[Mho]Retire has to be greater than 0, because the code returns an error if the
   st. dev. of shocks is 0 while using MoM. *)
If[NoMoM==False && \[Mho]Retire==0, Print["If MoM is used, then \[Mho] has to be non-zero."];Abort[]]
(* Default *)
If[Sdfix==True,
\[Sigma]\[Theta] = Join[Table[0.1,{i,1,65-25}],Table[0,{i,1,90-65}]];
\[Sigma]\[CapitalPsi] = Join[Table[0.1,{i,1,65-25}],Table[0,{i,1,90-65}]];]
(* NOTE:
   These age-varying st. dev. of income shocks are the same as in the final Matlab code. *)
If[Sdfix==False,
\[Sigma]\[Theta] = Join[Table[Sqrt[0.14-(0.14-0.03)/(PeriodsToSolve-(90-65)-1)*(i-1)],{i,1,65-25}],Table[0,{i,1,90-65}]];
\[Sigma]\[CapitalPsi] = Join[Table[Sqrt[0.075-(0.075-0.03)/(PeriodsToSolve-(90-65)-20-1)*(i-1)],{i,1,20}],Table[Sqrt[0.03+(0.08-0.03)/20*(i)],{i,1,20}],Table[0,{i,1,90-65}]];
(*
...
Constrained = True; (* Liquidity constraint: True or False *)
...
(* This loop will run from t=1 to t=40, i.e. a=26 to a=65. *)
...
(* If \[Sigma]\[Theta]Age=0, we need to fill in (NumOf\[Theta]ShockPoints-1) zeros to keep the matrix size fixed*)
If [\[Sigma]\[Theta]Age==0,\[Theta]ValsWork=Flatten[Append[\[Theta]ValsWork,ConstantArray[0,NumOf\[Theta]ShockPoints-1]]];\[Theta]ProbsWork=Flatten[Append[\[Theta]ProbsWork,ConstantArray[0,NumOf\[Theta]ShockPoints-1]]];];

If[\[Mho]>0,\[Theta]ValsWork=Prepend[(1/(1-\[Mho])) \[Theta]ValsWork,0];\[Theta]ProbsWork=Prepend[(1-\[Mho]) \[Theta]ProbsWork,\[Mho]]; \[Theta]Age = {\[Theta]ValsWork,\[Theta]ProbsWork}];

\[Theta]Age = {\[Theta]ValsWork,\[Theta]ProbsWork};
Return[\[Theta]Age];
];

(* This loop will run from t=1 to t=25, i.e. a=66 to a=90. *)
\[Theta]AgeOld[t_] := Block[{\[Sigma]\[Theta]Retire,\[Theta]Age,\[Theta]ValsRetire,\[Theta]ProbsRetire},
\[Sigma]\[Theta]Retire=\[Sigma]\[Theta][[65-(90-65)+t]];

{\[Theta]ValsRetire, \[Theta]ProbsRetire}=DiscreteMeanOneLogNormal[\[Sigma]\[Theta]Retire,NumOf\[Theta]ShockPoints];

\[Theta]ValsRetire=Flatten[Append[\[Theta]ValsRetire,ConstantArray[0,NumOf\[Theta]ShockPoints-Length[\[Theta]ValsRetire]+Boole[\[Mho]>0]-Boole[\[Mho]Retire>0]]]];
\[Theta]ProbsRetire=Flatten[Append[\[Theta]ProbsRetire,ConstantArray[0,NumOf\[Theta]ShockPoints-Length[\[Theta]ProbsRetire]+Boole[\[Mho]>0]-Boole[\[Mho]Retire>0]]]];

(* Assuming that \[Mho]Retire>0 only if \[Mho]>0 *)
If[\[Mho]Retire>0,
...
\[Theta]MatYoungToOld=Join[Table[\[Theta]AgeYoung[t], {t,1,PeriodsToSolve-(90-65)}]
, Table[\[Theta]AgeOld[t], {t,1,90-65}]];
\[Theta]Mat=Reverse[\[Theta]MatYoungToOld];

(*
(* NOTE: SAK 08/14/16. The following lines do not work if \[Mho]=0  *)
(* MNW: Next three lines are necessary to keep lists of even length*)
(* This makes GPU programming much easier *)
\[Theta]ValsRetire = Join[\[Theta]ValsRetire,Table[0,{NumOf\[Theta]ShockPoints+1-Length[\[Theta]ValsRetire]}]];
\[Theta]ProbsRetire = Join[\[Theta]ProbsRetire,Table[0,{NumOf\[Theta]ShockPoints+1-Length[\[Theta]ProbsRetire]}]];
\[Theta]Age = {\[Theta]ValsRetire,\[Theta]ProbsRetire};
...
\[CapitalPsi]ValsRetire = Join[\[CapitalPsi]ValsRetire,Table[1,{NumOf\[CapitalPsi]ShockPoints - Length[\[CapitalPsi]ValsRetire]}]];
\[CapitalPsi]ProbsRetire = Join[\[CapitalPsi]ProbsRetire,Table[0,{NumOf\[CapitalPsi]ShockPoints - Length[\[CapitalPsi]ProbsRetire]}]];
...
(* Put the data in the format used for the GPU version *)
\[Theta]ValsLife = Table[\[Theta]Mat[[t, 1]],{t,1,PeriodsToSolve}];
\[Theta]ProbsLife = Table[\[Theta]Mat[[t, 2]],{t,1,PeriodsToSolve}];
\[CapitalPsi]ValsLife = Table[\[CapitalPsi]Mat[[t, 1]],{t,1,PeriodsToSolve}];
\[CapitalPsi]ProbsLife = Table[\[CapitalPsi]Mat[[t, 2]],{t,1,PeriodsToSolve}];
...
RLife   = { If[UseGPU,0,Null] }; (* Interest factor for last period of life *)
\[Beta]Life   = { If[UseGPU,0,Null] }; (* Time preference factor for last period of life *)
\[ScriptCapitalG]Life = { If[UseGPU,0,Null] }; (* Income growth factor for last period of life *)
...
ClearAll[\[ScriptC]FuncLifeNoMoM, \[ScriptC]FuncLife, \[ScriptV]FuncLife, \[GothicC]FuncLife
...
If[NoMoM==True,
<<functions_stableNoMoM.m;
<<AddNewPeriodToParamLifeDatesNoMoM.m;
<<AddNewPeriodToSolvedLifeDatesNoMoM.m;
<<functions_ConsNValNoMoM.m;
,
<<functions_stable.m;
...
If[NoMoM==True,
  Do[AddNewPeriodToParamLifeDatesNoMoM[\[Rho]], {1}];
  Do[AddNewPeriodToSolvedLifeDatesNoMoM[\[Rho]], {1}];
,
  Do[AddNewPeriodToParamLifeDates[\[Rho]], {1}];
  Do[AddNewPeriodToSolvedLifeDates[\[Rho]], {1}]; (* else *)
  Do[AddNewPeriodToSolvedLifeDatesPesReaOpt[\[Rho]], {1}];];
];

(* begin{CDCPrivate} (* MNW Eliminated TUB *)
<<functions_stable.m;
<<AddNewPeriodToParamLifeDates.m;
...
(* MNW transformation of FindCubicSpline to the [0,1] interval replaces old version *)
FindCubicSpline[Bot\[Mu]_,Bot\[Chi]_,Bot\[Chi]d\[Mu]a_,Top\[Mu]_,Top\[Chi]_,Top\[Chi]d\[Mu]a_] := Block[{Bot\[Chi]d\[Mu],Top\[Chi]d\[Mu],X},
Bot\[Chi]d\[Mu] = Bot\[Chi]d\[Mu]a*(Top\[Mu] - Bot\[Mu]);
Top\[Chi]d\[Mu] = Top\[Chi]d\[Mu]a*(Top\[Mu] - Bot\[Mu]);
X = {Bot\[Chi],Bot\[Chi]d\[Mu],3*(Top\[Chi] - Bot\[Chi]) - 2*Bot\[Chi]d\[Mu] - Top\[Chi]d\[Mu],Bot\[Chi]d\[Mu] + Top\[Chi]d\[Mu] + 2*(Bot\[Chi] - Top\[Chi])};
Return[X];
];
...
(* MNW test code to see if transformation was working correctly -- can be deleted *)

SplineConFunc[m_] := Block[{j,Seg,b0,b1,b2,b3,x,Chi,Q,c,Mu},
Seg = 0;
Mu = Log[m - 0];
For[j=1,j<=Length[MuGridNow],j++,
  If[Mu > MuGridNow[[j]],Seg++]
];
b0 = CoeffsNow[[Seg+1,1]];
b1 = CoeffsNow[[Seg+1,2]];
b2 = CoeffsNow[[Seg+1,3]];
b3 = CoeffsNow[[Seg+1,4]];
x = If[(Seg > 0) && (Seg < Length[MuGridNow]),(Mu - MuGridNow[[Seg]])/(MuGridNow[[Seg+1]] - MuGridNow[[Seg]]),Mu];
Chi = b0 + x*(b1 + x*(b2 + x*(b3)));
Q = 1/(1 + Exp[Chi]);
c = MPCmin*(m + (1 - Q)*Deltah);
Return[c];
];
