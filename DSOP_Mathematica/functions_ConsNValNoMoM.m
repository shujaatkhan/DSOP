...
ClearAll[\[ScriptC]NoMoMVec];
\[ScriptC]NoMoMVec[mVect_?VectorQ, PeriodsUntilT_]:=Block[{mLowerBoundt},
If[PeriodsUntilT == 0, Return[Transpose[{mVect}]]
, mLowerBoundt = \[GothicA]LowerBoundLife[[PeriodsUntilT+1]]; 

InterpFuncNext = \[ScriptC]FuncLifeNoMoM[[PeriodsUntilT+1]];
cVectNoMoM = InterpFuncNext[mVect];
(* NOTE: No liquidity constraint *)
  Return[Transpose[{cVectNoMoM}]];
];
];(*End Block*)
...
ListOptionConsFnInSim={{"cInterpNoMoM"}
, {"cInterp"}
...
FuncToDefine\[ScriptC]VecN\[Chi]Vec[OptionConsFnInSim_]:=Block[{}
, 
ClearAll[\[Chi]Vec];
ClearAll[\[ScriptC]Vec];
(* This is to call the Interpolated function \[ScriptC]FuncLifeNoMoM[[]], which does not use the MoM. Very inaccurate but Fast! *)
If[OptionConsFnInSim=={"cInterpNoMoM"}
, \[ScriptC]Vec[mVect_?VectorQ, PeriodsUntilT_]:=\[ScriptC]FuncLifeNoMoM[[PeriodsUntilT+1]][mVect]
];
