...
(* This augments the vector to transition the cubic spline to linear outside the boundaries; not needed now
...
(* Construct below first gridpoint, then join to internal gridpoints, then join last gridpoint *)
...
\[Chi]\[Mu]Coeffs = Partition[\[Chi]\[Mu]Coeffs,4];

  AppendTo[\[Mu]GridsLife, \[Mu]Vect];
